...
router.post('/login', (req, resp, next) => {
    ...
    passport.authenticate('local', (authErr, user, verifyOptions) => {
        console.log('passport auth callback:', authErr, user ? user.email : null, verifyOptions);
        const verifyMessage = verifyOptions != null ? verifyOptions.message : null;

        if (authErr) {
            handleError('login authenticate', authErr);
        ...
        //create a login session after a successful auth
        req.login(user, err => {
            if (err) {
